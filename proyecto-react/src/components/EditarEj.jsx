...
export function EditarEj(){
    const history = useHistory();
    const ID = new URLSearchParams(useLocation().search).get("Ejercicio");
    ...
    const handleSubmit = (e) =>{
        ...
        if(radioSel === "0"){
            params = new URLSearchParams({id: ID, type: radioSel, menor: menor0, mayor: mayor0});
        } else if(radioSel === "1"){
            params = new URLSearchParams({id: ID, type: radioSel, menor: menor1, mayor: mayor1});
        } else{
            params = new URLSearchParams({id: ID, type: radioSel, menor: menor2, mayor: mayor2});
        }

        fetch("http://localhost:8080/Proyecto/ServletEditar?"+params).then( res => res.json() )
        .then( data => {
    ...
    return(
        <div className={estilos.contenedor}>
            ...
            <form className={estilos.form} onSubmit={handleSubmit} onReset={handleReset}>
                <p>
                    <input id="0" type="radio" defaultChecked onChange={handleChange} name="P"></input>
                    P( <input 
                            type="number" 
                            disabled={radioSel !== "0"} 
